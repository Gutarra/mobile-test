...
public class GenerateWord {
    ...
    public void addImageToWord(AppiumDriver<MobileElement> driver) throws IOException {
        InputStream inputStream = null;
        try {
            ...
            double w, h,wp,hp;
            w = Double.parseDouble(driver.getCapabilities().asMap().get("deviceScreenSize").toString().split("x")[0]);
            h = Double.parseDouble(driver.getCapabilities().asMap().get("deviceScreenSize").toString().split("x")[1]);

            wp = 214;
            hp = (wp/w) * h;

            run.addPicture(inputStream, Document.PICTURE_TYPE_PNG, "1", Units.toEMU(wp), Units.toEMU(hp));
